...
enable_sasl_config() {
  ...
  export SERVER_JVMFLAGS="-Dzookeeper.superUser=${ADMIN_USERNAME} -Dzookeeper.allowSaslFailedClients=false -Djavax.net.debug=ssl -Djdk.tls.client.protocols=TLSv1.2 -Djava.security.auth.login.config=${ZOOKEEPER_HOME}/conf/zookeeper_jaas.conf ${SERVER_JVMFLAGS}"
